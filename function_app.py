...
try:
    ...
    from src import (
        ...
        tts_processor,
        audio_processor,
        storage_client # Added storage_client
    )
    # Import specific libraries if needed directly
    try:
        import openai # Check if openai is importable early
        from azure.storage.blob import BlobServiceClient # Check storage import
    except ImportError:
        logging.error("OpenAI or Azure Storage library seems missing despite package structure import.")
        raise
    ...
    try:
        ...
        import storage_client # Added storage_client
...
def daily_email_podcast_job(myTimer: func.TimerRequest) -> None:
    ...
    try:
        ...
        logging.info(f"Starting upload of final audio file: {final_audio_path}")
        if not final_audio_path or not os.path.exists(final_audio_path):
            logging.error("Final audio path not valid or file does not exist. Aborting before upload.")
            raise RuntimeError("Failed to get valid final audio path from assembly step.")
        if not config.AZURE_STORAGE_CONNECTION_STRING or not config.AZURE_STORAGE_CONTAINER_NAME:
             logging.error("Azure Storage Connection String or Container Name missing in configuration. Cannot upload.")
             raise ValueError("Azure Storage configuration missing.")

        # Actual call to the storage client
        podcast_url = storage_client.upload_blob_and_get_sas_url(
            local_file_path=final_audio_path,
            connection_string=config.AZURE_STORAGE_CONNECTION_STRING,
            container_name=config.AZURE_STORAGE_CONTAINER_NAME,
            # sas_expiry_days=config.SAS_EXPIRY_DAYS # Optional: configure expiry via config.py
        )

        if not podcast_url:
             logging.error("Failed to upload file to Azure Blob Storage or generate SAS URL. Aborting.")
             raise RuntimeError("Azure Storage upload/SAS generation failed.")

        logging.info(f"Upload complete. Podcast SAS URL obtained.") # Don't log SAS URL itself


        # --- Step 8: Send Email Notification ---
        if podcast_url:
            logging.info(f"Preparing notification email...") # Don't log URL
            ...
            if success:
                ...
                # Consider if this failure should halt the function or just be logged
        # Removed the 'else' block as upload failure now raises an error earlier

    except (ConnectionError, ValueError, RuntimeError, openai.APIError) as e:
         # Catch specific, expected exceptions from our workflow or OpenAI/Azure
        ...
        if temp_tts_dir and os.path.isdir(temp_tts_dir):
            ...
            # else: # Commented out for less verbose logging unless debugging
